...
try {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    sockets.push(socket);
    socket.onmatchdata = event => { states[i] = JSON.parse(new TextDecoder().decode(event.data)); };
  ...
  console.log('PASS: two players joined, ready/start, Korean composition, sentence confirmation and next sentence synchronized.');
